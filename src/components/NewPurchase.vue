<template>
  <div>
    ...
    <el-form :model="filt" :inline="true" class="filt">
      ...
      <el-select v-model="filt.vender" placeholder="供应商" @change="venderGet">
        <el-option
...
export default {
  ...
  methods: {
    ...
    if(this.filt.Date[0] && this.filt.Date[1]){
      ...
      //开始日期
          let startDate = new Date(this.filt.Date[0])
           var resStartDate = startDate.getFullYear() + '-' + p((startDate.getMonth() + 1)) + '-' + p(startDate.getDate())
  
      //结束日期
      let endDate = new Date(this.filt.Date[1])
          var resEndDate = endDate.getFullYear() + '-' + p((endDate.getMonth() + 1)) + '-' + p(endDate.getDate())
    }else{
      var resStartDate = ""
      var resEndDate = ""
    }
    ...
    console.log(Number(this.filt.poId),this.filt.vender,this.filt.payType);

       axios
        .post(`/api/main/purchase/pomain/query`,`poId=${this.filt.poId}&venderCode=${this.filt.vender}&startDate=${resStartDate}&endDate=${resEndDate}&payType=${this.filt.payType}&status=1&page=1`)
        .then(resp => {
    ...
    //添加
    btnadd() {
      this.$router.push({ path: "/Addorders" });
    },
    //删除
    handleDelete(index, row) {
      this.$confirm(
        ...
        .then(() => {
          axios
            .post(
              `/api/main/purchase/pomain/delete`,
              `poId=${row.poId}&page=${this.currPage}`
            )
            .then(resp => {
              //删除后回到该页
              axios
    ...
    handleEdit(index, row) {
      this.$router.push({ path: "/AlterOrders" });
      //把值传入store的alterorders函数里面
      this.$store.dispatch({
        type: "alterorders",
        index: index,
        row: row
