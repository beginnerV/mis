  ...
  <div>
    ...
    <el-tabs type="card" @tab-click="pay" class="pay">
      ...
      >
        ...
        <el-table-column prop="prePayFee" label="处理状态" :formatter="getstatus"></el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope" class="btnmini">
            <el-button size="mini" @click="handleEdit(scope.$index, scope.row)" type="primary">入库</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row) {
      this.$confirm(
        ...
        .then(() => {
           console.log(row.poId,row.type,row.paytype,row);
          axios
            .post(
              `/api/main/stock/instock`,
              `poId=${row.poId}&type=${row.status}&payType=${row.payType}`
            )
            .then(resp => {
  ...
  if (row.status == 1) {
    ...
    pay(tab, event) {
      ...
      if (tab.label == "货到付款") {
        axios
          .post(`/api/main/purchase/pomain/show`, `type=3&payType=1`)
          .then(resp => {
        ...
        axios
          .post(`/api/main/purchase/pomain/show`, `type=3&payType=2`)
          .then(resp => {
        ...
        axios
          .post(`/api/main/purchase/pomain/show`, `type=3&payType=3`)
          .then(resp => {
