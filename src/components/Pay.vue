      ...
      >
        ...
        <el-table-column label="操作">
          <template slot-scope="scope" class="btnmini">
            <el-button size="mini" @click="handleEdit(scope.$index, scope.row)" type="primary">付款</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
  ...
  methods: {
    ...
    handleEdit(index, row) {
      this.$confirm(
        ...
        type = 2;  //如果状态是1，付预付款
      } else if(row.payType == 3 && row.status == 2){
        type = 1;  //如果状态是2，付尾款
      }
         ...
         axios
          .post(
            ...
            `type=4&payType=${this.paytype}&page=${this.currPage}`
          )
          .then(resp => {
...
console.log(row);
  ...
  if (row.payType == 1) {
    ...
    pay(tab, event) {
      ...
      if (tab.label == "货到付款") {
        axios
          .post(`/api/main/purchase/pomain/show`, `type=4&payType=1`)
          .then(resp => {
        ...
        axios
          .post(`/api/main/purchase/pomain/show`, `type=4&payType=2`)
          .then(resp => {
        ...
        axios
          .post(`/api/main/purchase/pomain/show`, `type=4&payType=3`)
          .then(resp => {
    ...
    goPage(currPage) {
      ...
      if (this.paytype == 1) {
        axios
          .post(
            `/api/main/purchase/pomain/show`,
            `type=4&payType=1&page=${this.currPage}`
          )
          .then(resp => {
            this.pageNum = resp.data.pageNum;
            this.users = resp.data.list;
        ...
        axios
          .post(
            `/api/main/purchase/pomain/show`,
            `type=4&payType=2&page=${this.currPage}`
          )
          .then(resp => {
            this.pageNum = resp.data.pageNum;
            this.users = resp.data.list;
        ...
        axios
          .post(
            `/api/main/purchase/pomain/show`,
            `type=4&payType=3&page=${this.currPage}`
          )
          .then(resp => {
            console.log(111);
            this.pageNum = resp.data.pageNum;
